...
class YOLOV4Loss(nn.Module):
    ...
    def forward(self, preds, targets, idx, img_size):
        ...
        # tx, ty, tw, th
        preds_txty = (torch.sigmoid(preds_permute[..., 0:2]) * self.scale_x_y - 0.5 * (self.scale_x_y - 1)  + grid) / grid_size
        ...
        loss_bbox = (1. - bbox_iou(preds_bbox_mask, targets_bbox_mask, CIoU=True)).sum()
        ...
        for b in range(batch_size):
            ...
            for target_batch in targets_batch:
                ...
                if anchors_ious_index in self.masks[idx]:
                    ...
                    # if obj_mask[b, find_obj_index, j, i]:
                    #     continue
